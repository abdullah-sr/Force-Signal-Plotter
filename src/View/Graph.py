...
import numpy
 ...
 ## MathPlotLib canvas for plotting the graphs
class MplCanvas(FigureCanvas):
    """Ultimately, this is a QWidget which is added on the GUI."""
    def __init__(self, parent=None, width=5, height=4):
        fig = Figure(figsize=(width, height), facecolor= "white")
        self.axes = fig.add_subplot(1,1,1)  # 1 by 1 grid
...
class AnimationWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.navi_toolbar = NavigationToolbar(self.canvas, self) ## adds the control buttons above the graph
        ...
        self.xlimit = (0,20) ## 0-20 sec
        self.ylimit = (0,200) ## 0-200 kg
        self.data_points = 0.03 ## corresponds to 30 ms
        ...
        self.ani = ControlFuncAnimation(self.canvas.figure, self.update_graph, init_func=self.init, blit=True, interval=30) ## update graph every 30 ms
    ...
    def update_graph(self, i):
        ...
        xdata = numpy.array(range(len(data)))*self.data_points
        self.line.set_data(xdata, data)
        if(xdata[-1] >= 20): ## stop collecting data at 20 seconds
            temp = self.line,
            self.stop()
            return temp
        return self.line,
    ...
    def stop(self):
        ...
        self.line, = self.canvas.axes.plot(numpy.array(range(len(data)))*self.data_points, data, lineWidth=2)
        self.canvas.axes.set_xlabel(self.xlabel)
        self.canvas.axes.set_ylabel(self.ylabel)
        self.canvas.axes.set_xlim(self.xlimit)
        self.canvas.axes.set_ylim(self.ylimit)
    ...
    # load data from a cvs file and graph it
    def loadData(self):
        x=deque()
        y=deque()
        even=True
...
#a modification to the animation class to allow starting and stopping
